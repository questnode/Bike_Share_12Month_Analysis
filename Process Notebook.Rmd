...
After initial exploratory exercise, there are a few tasks to be done before putting the data into analysis models.
...
The number of unique station names and IDs should be an exact match.  If there are data collection errors, some station names may be sorted into the wrong station IDs.  If that's the case, then we will need to compile a list of correct staion IDs along with their names, and use it to correct the wrong entries in the main dataset. 

```{r}
# First we will check if there are rows filled with station ID, but empty station name.  And vice-versa.  
# In this case, we have no such error.  However, if we did, then the subset can be relocated into another dataframe for processing.
...
# The second test is done with the number of unique IDs against the number of unique names.  Those two number should match.  We will do this will a combined dataset from both start station and end station.
#   Note that this does not guarantee that there may be cases where the number of erroneous station names is the same as the number of erroneous station  IDs.  
#   In such cases, this test will still give a passing result. However this test is a quick first test.
...
# Since there is a mismatch, the next step is to find all unique IDs with multiple station names.  
# This will be stored in a separate dataframe so to be used as correction list later.
...
#add_list <- (data.frame(c('13042'),c('Michigan Ave & Oak Street')) %>% 'colnames<-' (c('id','name')))
#work_list <- rbind(work_list, add_list)
  
#counter <- 0

correction_list <- data.frame(matrix(nrow=0, ncol=3))
colnames(correction_list) <- c('id','name','Freq')
weird_list <- data.frame(matrix(nrow=0, ncol=3))
colnames(weird_list) <- c('id','name','Freq')
for (i in 1:nrow(unique_ids)) {
  ...
  if (nrow(id_to_unify)>1 & nrow(id_to_unify)<=10) {
    # weird_list <- rbind(weird_list,id_to_unify)
    id_to_unify <- id_to_unify[(1:1),]
    correction_list <- rbind(correction_list,id_to_unify)
  # } else if (nrow(id_to_unify)>10) {
  #   weird_list <- rbind(weird_list,id_to_unify)
    
  # } else if (nrow(id_to_unify)>1 & id_to_unify[(1:1),'Freq']==1) {
  #   correction_list <- rbind(correction_list,id_to_unify)
...
#holding_list_name_id$okname <- ifelse(!is.na(holding_list_name_id$name), holding_list_name_id$name, merged_trips$start_station_name)
holding_list_name_id$name <- coalesce(holding_list_name_id$name, merged_trips$start_station_name)
#merged_trips$old_start_station_name <- merged_trips$start_station_name
...
#holding_list_name_id$name <- ifelse(!is.na(holding_list_name_id$name), holding_list_name_id$name, merged_trips$end_station_name)
holding_list_name_id$name <- coalesce(holding_list_name_id$name, merged_trips$end_station_name)
#merged_trips$old_end_station_name <- merged_trips$end_station_name
...
test <- data.frame(table(work_list[(work_list$id == unique_ids[1,]),]))
...
Earlier data did not contain station IDs, but has geo-coordinate that may be used reference to later data
...
# merged_trips$old_start_station_id <- merged_trips$start_station_id
# merged_trips$new_start_station_id <- ifelse(!merged_trips$start_station_id == "", merged_trips$start_station_id, start_station_ids_found)
# merged_trips$old_start_station_name <- merged_trips$start_station_name
# merged_trips$new_start_station_name <- ifelse(!merged_trips$start_station_name == "", merged_trips$start_station_name, start_station_names_found)
...
# merged_trips$old_end_station_id <- merged_trips$end_station_id
# merged_trips$new_end_station_id <- ifelse(!merged_trips$end_station_id == "", merged_trips$end_station_id, end_station_ids_found)
# merged_trips$old_end_station_name <- merged_trips$end_station_name
# merged_trips$new_end_station_name <- ifelse(!merged_trips$end_station_name == "", merged_trips$end_station_name, end_station_names_found)

merged_trips$end_station_id <- ifelse(!merged_trips$end_station_id == "", merged_trips$end_station_id, end_station_ids_found)
merged_trips$end_station_name <- ifelse(!merged_trips$end_station_name == "", merged_trips$end_station_name, end_station_names_found)

```
### Add a column to calculate duration of bike rental

```{r}
merged_trips$duration <- round(difftime(as.POSIXct(merged_trips$ended_at,format="%Y-%m-%d %H:%M:%S"),as.POSIXct(merged_trips$started_at,format="%Y-%m-%d %H:%M:%S"),unit="mins"),1)
...
station_stat <- start_station_usage

end_station_usage <- data.frame(sort(table(merged_trips$end_station_name),decreasing=T))
colnames(end_station_usage) <- c('name','freq_as_end')

station_stat <- unique((inner_join(station_stat, select(end_station_usage, c("name","freq_as_end")), by = c("name"))))

colnames(station_stat) <- c('start_station_name','freq_as_start','freq_as_end')
station_stat <- unique((inner_join(station_stat, select(merged_trips, c("start_station_name","start_station_id")), by = c("start_station_name"))))

test <- data.frame(sort(unique(station_stat$start_station_id)))
# df3 <- unique((inner_join(df1, select(df2, c("a","c")), by = c("a"))))


ggplot(data = sort(table(start_station_name)) +
